...
import AIaaS.AIHub_Mgmt.multiProcess as multiProcess
...
def on_message(client, userdata, msg): # 서버에게서 PUBLISH 메시지를 받을 때 호출되는 콜백
    ...
    for key in msg_type.keys():
        if key == "m2m:cin":
            con = msg_type["m2m:cin"]["con"]

            if con["ReqID"] == 0:
                con_info = con["Info"]

                for i in range (len(con_info)):
                    con_sensor = con_info[i]["sensor"]
                    con_cf = con_info[i]["cf"]
                    
                    multiProcess.multiprocess(con_sensor, con_cf)
                    print(con_sensor+"multiprocess")
...
def subscribing():
    ...
    client.subscribe('/oneM2M/req/+/AIHubtarget/#')
